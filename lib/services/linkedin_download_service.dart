...
class LinkedInDownloadService {
  LinkedInDownloadService({Dio? dio}) : _dio = dio ?? Dio();
  ...
  Future<String> download(
    ...
    final mediaUrl = await _resolveMediaUrl(linkedinUrl);
    final fileName = 'linkedin_${DateTime.now().millisecondsSinceEpoch}.mp4';
  ...
  Future<String> _resolveMediaUrl(String linkedinUrl) async {
    try {
      final response = await _dio.post<Map<String, dynamic>>(
        resolverEndpoint,
        data: {'url': linkedinUrl, 'downloadMode': 'auto'},
        options: Options(
          headers: {
      ...
      throw const DownloadException(
        'This link did not return a downloadable public LinkedIn video.',
      );
    } on DioException catch (error) {
      ...
      throw DownloadException(
        'Could not resolve this LinkedIn link (${error.response?.statusCode ?? 'network error'}).',
      );
    }
  }
}
